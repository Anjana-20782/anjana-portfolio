...
const experiences = [
  ...
  // You can add more objects here for future jobs
];
  ...
  return (
    <section id="experience" className="py-24 px-6 md:px-20 relative bg-[color:var(--background)]">
      ...
      <div className="absolute inset-0 z-0 opacity-15 pointer-events-none">
        <DotGrid dotSize={2} gap={35} baseColor="#6366f1" />
      </div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-8 min-h-[400px]">
          {/* Left Side: Sidebar Navigation (Company List) */}
          <div className="flex md:flex-col overflow-x-auto md:overflow-visible border-l-2 md:border-l border-purple-500/20 md:w-64">
            {experiences.map((exp) => (
              <button
                key={exp.id}
                onClick={() => setActiveTab(exp.id)}
                className={`text-left px-6 py-4 text-sm font-mono whitespace-nowrap transition-all duration-300 border-b-2 md:border-b-0 md:border-l-2 -ml-[2px] ${
                  activeTab === exp.id
